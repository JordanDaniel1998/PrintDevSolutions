<div class="flex flex-col gap-10 w-11/12 mx-auto">
    <div>
        ...
        <p class="text-[#DDDDDD] font-inter font-medium text-text12 capitalize">
            {{ $posts->count() ? '' : 'Sé el primero en comentar' }}</p>
    </div>
    ...
    <div>
        <form action="" method="POST" wire:submit.prevent="saveComment" novalidate class="flex flex-col gap-5">
            ...
            @endauth
            @guest
                <div class="text-gray-600 font-inter font-medium text-text16 first-letter">
                    <span>Para poder comentar necesitar estar logueado,</span>
                    <a href="{{ route('login') }}" class="underline text-blue-600">Click aquí</a>
                    <span class="w-8"> &#128522; </span>
                </div>
            @endguest
